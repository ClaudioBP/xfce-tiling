...
class TilingDaemon:
    ...
    def run(self):
        ...
        if not Keybinder.bind(self.hotkey, self._on_hotkey):
            print(f"[xfce-tiling] no se pudo registrar el atajo {self.hotkey}")
        else:
            print(f"[xfce-tiling] atajo global: {self.hotkey}")
        GLib.timeout_add(POLL_MS, self._tick)
        print("[xfce-tiling] demonio en marcha (hover + atajo)")
        Gtk.main()
