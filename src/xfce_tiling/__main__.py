...
Uso:
    python -m xfce_tiling            # arranca el demonio (hover + atajo)
    python -m xfce_tiling daemon     # idem
    python -m xfce_tiling menu       # muestra el menú una vez (ventana activa)
    python -m xfce_tiling --version
"""
...
def _show_menu_once():
    ...
    if not wm.is_tileable(target):
        print("[xfce-tiling] la ventana activa no admite tiling")
        return
...
def main(argv=None):
    ...
    if cmd in ("--version", "-v"):
        print(f"xfce-tiling {__version__}")
        return
    if cmd == "menu":
        _show_menu_once()
        return
    ...
    print(f"[xfce-tiling] orden desconocida: {cmd}")
    sys.exit(2)
