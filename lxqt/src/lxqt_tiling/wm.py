...
def frame_extents(win: Wnck.Window) -> tuple[int, int, int, int]:
    ...
    openbox (extents > 0) y para ventanas CSD (todo 0).
    """
...
def has_server_decorations(win: Wnck.Window) -> bool:
    """True si openbox dibuja la decoración (por tanto el botón maximizar)."""
...
# Medido en openbox 3.6: la decoración se actualiza ~35 ms tras desmaximizar.
...
def _settled_frame_extents(xid: int, stale: tuple[int, int, int, int] | None):
    ...
    Al desmaximizar, openbox devuelve los bordes normales unos 30 ms después.
...
# Dos repasos: el primero llega a tiempo para el desfase de openbox y el
...
def _verify_geometry(win: Wnck.Window, zone: tuple[int, int, int, int], serial: int) -> bool:
    ...
    Hace falta porque justo después de desmaximizar ni openbox ni la propia
...
def apply_zone(win: Wnck.Window, x: int, y: int, w: int, h: int) -> None:
    ...
    tras desmaximizar, que openbox convierte con la decoración de la ventana
    ...
    if was_constrained:
        # openbox aplica mal la primera petición que sigue a un unmaximize:
