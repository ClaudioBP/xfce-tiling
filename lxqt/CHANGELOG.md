...
- LXQt/Openbox flavor: this directory is a fork of the XFCE project renamed
  to `lxqt-tiling` (`lxqt_tiling`). The maximize-button hover detection is
  now driven by openbox's `titleLayout` (rc.xml) and theme (`themerc`)
  instead of xfwm4's `button_layout`, deriving the button geometry from each
  window's real `_NET_FRAME_EXTENTS` (`openbox_button.py`).
